...
#nohup ${TIBCO_TRA_HOME_PATH}/bin/AppManage -stop -app INT/pam_nms_ndb2crm -domain $TIB_DOMAIN -cred $CRED_FILE >> nohup.out.ESB 2>&1

echo Starting required PAM/wrapper instances...

nohup ${TIBCO_TRA_HOME_PATH}/bin/AppManage -start -app INT/pam_nms_ndb2crm -domain $TIB_DOMAIN -cred $CRED_FILE >> nohup.out.ESB 2>&1
if [[ $OPTION == "STUB" ]]; then
	nohup ${TIBCO_TRA_HOME_PATH}/bin/AppManage -start -app STUB/pam_nms_ndb2crm -domain $TIB_DOMAIN -cred $CRED_FILE >> nohup.out.ESB 2>&1
 	nohup ${TIBCO_TRA_HOME_PATH}/bin/AppManage -start -app STUB/esb2StubPam -domain $TIB_DOMAIN -cred $CRED_FILE >> nohup.out.ESB 2>&1
fi
if [[ $OPTION == "ESB" ]]; then
